...
def parse_language_data(language_code, batch_size=1000, start_idx=0, end_idx=None):
    ...
    input_path = f"data/{FILTERED_BY_MEDIAN_AND_STD_PATH}/{language_code}_embeds_filtered.tsv"
        ...
        with open(output_path, file_mode, encoding='utf-8') as f_out:
            ...
            for chunk in reader:
                ...
                for idx, row in chunk_subset.iterrows():
                    ...
                    register = str(row.get('register', '')).strip() if 'register' in row else 'IN'
                ...
                sys.stdout.write(f"\rProcessed: {total_processed} texts, Progress: {progress}%, Errors: {errors}")
                sys.stdout.flush()
